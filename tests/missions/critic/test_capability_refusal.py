...
019f1032, 2026-06-14: "book me a trip from Exampleville to Tokyo"). We now route a
...
_REFUSAL = (
    ...
    "so I'm unable to book a trip from Exampleville to Tokyo for you."
)
...
def test_refusal_answer_detects_honest_capability_refusal() -> None:
    out = capability_refusal_answer(
        _result_stream(_REFUSAL), prompt="book me a trip from Exampleville to Tokyo"
    )
...
async def test_runner_rejects_impossible_task_in_one_shot(tmp_path: Path) -> None:
    verdict = await CriticRunner().run(
        mission_prompt="book me a trip from Exampleville to Tokyo",
        worker_diff="",
...
async def test_runner_reject_does_not_overflow_verdict_summary(tmp_path: Path) -> None:
    ...
    long_refusal = (
        ...
        "book this trip from Exampleville to Tokyo end to end for you here."
    )
