"""Unit tests for `jarvis.memory.people.PersonStore` using synthetic people."""
...
class TestGetOrCreate:
    def test_creates_file_for_new_person(
        ...
        person = person_store.get_or_create("Jordan", relationship="colleague")
        assert person.path.exists()
        assert person.path.stem == person_slug("Jordan")
    ...
    def test_same_name_returns_same_file_no_duplicate(
        ...
        p1 = person_store.get_or_create("Jordan", relationship="colleague")
        p2 = person_store.get_or_create("Jordan", relationship="colleague")
    ...
    def test_umlauts_in_name_become_slug(self, person_store: PersonStore) -> None:
        p = person_store.get_or_create("Jordan Müller", relationship="colleague")  # i18n-allow: synthetic umlaut name, matched by slug normalization
        assert p.path.stem == "jordan_mueller"
...
class TestFindByAlias:
    def test_finds_by_exact_slug(self, person_store: PersonStore) -> None:
        ...
        found = person_store.find_by_alias("Jordan")
        assert found is not None
        assert found.name == "Jordan"
