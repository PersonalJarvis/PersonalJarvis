...
def address_book(env):
    ...
    "Testoria Novak" and "Testor Novak" share no e-mail, phone or contact slug,
...
def test_people_can_be_filtered_by_name_or_by_identifier(env, address_book):
    ...
    by_name = people(env, q="testoria")
    by_email = people(env, q="testoria@example.com")
    by_phone = people(env, q="9999")

    assert [p["display_name"] for p in by_name] == ["Testoria Novak"]
    assert [p["display_name"] for p in by_email] == ["Testoria Novak"]
    assert [p["display_name"] for p in by_phone] == ["Testor Novak"]
...
def test_person_profile_carries_every_identifier_and_the_open_proposal(
    ...
    seed(env)
    testoria = next(p for p in people(env) if p["display_name"] == "Testoria Novak")

    response = env.client.get(f"/api/ultrawiki/identity/people/{testoria['id']}")

    assert response.status_code == 200, response.text
    profile = response.json()["person"]
    assert response.json()["forwarded"] is False
    assert profile["emails"] == ["testoria@example.com"]
    assert profile["phones"] == ["+4915123456789"]
    assert "Test" in profile["names"]
