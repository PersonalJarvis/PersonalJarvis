...
Konvention: Fakes statt Mocks (CLAUDE.md). Echtes ``git init`` im tmp-Path
fuer realistische Tests; Subprocess-Failure-Modes laufen ueber den
realen git-Binary, nicht ueber unittest.mock.

Subprocess-Setup-Helpers (``_git``, ``_commit``) sind sync und werden
bewusst aus den async Tests aufgerufen — das ist Test-Fixture-Setup,
kein Runtime-IO. Daher ``# noqa: ASYNC221``.
"""
...
# --- Sync subprocess-helpers (Test-Setup, kein Runtime-Code) ---


def _git(repo: Path, *args: str) -> subprocess.CompletedProcess[bytes]:
    """Run ``git -C <repo> <args>`` synchronously for test setup."""
    return subprocess.run(  # noqa: ASYNC221  (sync test setup, kein async runtime)
        ["git", "-C", str(repo), *args],
        check=True,
        capture_output=True,
    )
...
def _commit_init(repo: Path, filename: str = "init.txt") -> None:
    """Erste Commit fuer einen frisch initialisierten Repo (damit Branches existieren)."""
...
# --- Fixture: realer git-init im tmp-Path ---


@pytest.fixture
def git_repo(tmp_path: Path) -> Path:
    """Erstellt echtes git-Repo via subprocess (sync, tmp-Setup)."""
...
async def test_probe_branch_after_checkout(git_repo: Path) -> None:
    """Wechsle Branch und verifizere dass die Aenderung sichtbar ist."""
...
async def test_probe_detached_head_returns_sha_prefix(git_repo: Path) -> None:
    """Detached HEAD: returnt 8-char SHA-prefix statt None."""
    ...
    p = GitProbe()
    result = await p.probe(cwd=str(git_repo))
    # Erwartung: 8-char prefix, nicht None
...
async def test_probe_worktree_repo_resolves_gitdir(
    ...
    """Worktree: ``.git`` ist file mit ``gitdir: ...``-Inhalt."""
...
async def test_probe_invalid_cwd_returns_none() -> None:
    """Nicht-existierender Pfad -> None, kein Crash."""
    p = GitProbe()
...
async def test_probe_handles_corrupted_head_file(git_repo: Path) -> None:
    """``.git/HEAD`` mit Garbage -> None oder valider String, kein Crash."""
    head_file = git_repo / ".git" / "HEAD"
    head_file.write_text("\x00\x01\x02 not a valid HEAD")
    p = GitProbe()
    result = await p.probe(cwd=str(git_repo))
    # Sollte entweder via subprocess success returnen ODER None — beides OK,
...
async def test_probe_completes_under_total_budget(git_repo: Path) -> None:
    """Plan §9 AC: 200ms hard timeout pro Probe-Call."""
    p = GitProbe()
