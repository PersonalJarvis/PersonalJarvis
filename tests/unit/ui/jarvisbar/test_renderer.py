...
# --- HiDPI scaling: keep a constant PHYSICAL size on a scaled display --------


def test_renderer_scales_surface_with_display_dpi():
    # BUG (4K @ 150%): the bar drew a fixed 107x48 pixel window the DPI-aware
    # process no longer upscaled, shrinking the idle pill to a ~48x8 sliver that
    # could not be seen or grabbed. A scaled renderer must enlarge the whole
    # surface (and pill) so the bar keeps a constant PHYSICAL size on a scaled
    # display — this is the fix, keyed off the monitor's DPI.
    scaled = R.JarvisBarRenderer(scale=1.5)
    assert scaled.win_w == round(R.WIN_W * 1.5)
    assert scaled.win_h == round(R.WIN_H * 1.5)
    assert scaled.active_w == round(R.ACTIVE_W * 1.5)
    img = scaled.render(0.1, "idle", 0.0)
    assert img.size == (scaled.win_w, scaled.win_h)


def test_renderer_base_scale_is_unchanged():
    # 100% displays / existing behaviour: the default renderer is byte-for-byte
    # the old fixed size, so all the other renderer tests (and 100% users) stay
    # exactly as before.
    base = R.JarvisBarRenderer()
    assert base.win_w == R.WIN_W and base.win_h == R.WIN_H
    assert base.active_w == R.ACTIVE_W
    assert base.render(0.1, "idle", 0.0).size == (R.WIN_W, R.WIN_H)


def test_renderer_scale_never_shrinks_below_base():
    # A <1.0 scale would make the sliver worse, never better — clamp to the base
    # design size as the floor.
    assert R.JarvisBarRenderer(scale=0.5).win_w == R.WIN_W


def test_scaled_active_pill_grows_proportionally():
    base = R.JarvisBarRenderer()
    scaled = R.JarvisBarRenderer(scale=1.5)
    for _ in range(120):  # settle the eased pill size
        base.render(0.0, "speak", 0.0)
        scaled.render(0.0, "speak", 0.0)
    assert scaled._st.ph == pytest.approx(base._st.ph * 1.5, rel=0.05)
    assert scaled._st.pw == pytest.approx(base._st.pw * 1.5, rel=0.05)
