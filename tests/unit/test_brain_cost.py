"""Tests fuer jarvis.brain.cost — Pricing-Tabelle + calculate_cost_usd."""
...
class TestPricingTable:
    def test_table_has_canonical_models(self) -> None:
        # Smoke: die Modelle die der Sub-Jarvis-Tier in TIER_DEFAULTS_BY_PROVIDER
        # auflistet, muessen alle in der Pricing-Tabelle sein — sonst werden
        # Sub-Jarvis-Calls "kostenlos" statistiert.
        expected_models = [
        ...
        for m in expected_models:
            assert m in PRICING_USD_PER_MTOK, f"Pricing fehlt fuer {m}"

    def test_table_has_router_models(self) -> None:
        # Router-Tier — kostenrelevant fuer Cost-Reporting.
        expected_models = [
    ...
    def test_pricing_tuple_shape(self) -> None:
        for model, rates in PRICING_USD_PER_MTOK.items():
            assert isinstance(rates, tuple), f"{model}: rates ist kein Tuple"
            ...
            # Output sollte typischerweise teurer sein als Input
            # (oder gleich, bei Flat-Pricing). Wir warnen nicht, nur sanity.
