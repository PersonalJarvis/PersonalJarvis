...
def _surface() -> dict:
    return {
        "create_artifact": object(),  # the gated one
...
def test_nothing_else_is_touched():
    ...
    out = _mgr()._hide_artifact_tool_without_request(_surface(), "wie spät ist es")
...
def test_a_surface_without_the_tool_is_returned_untouched():
    ...
    assert _mgr()._hide_artifact_tool_without_request(surface, "hallo") is surface


def test_a_gate_fault_fails_open(monkeypatch):
    """A gate bug must never cost the brain a tool it was supposed to have."""
    import jarvis.brain.artifact_gate as gate

    def _boom(_text: str) -> bool:
        raise RuntimeError("gate exploded")

    monkeypatch.setattr(gate, "wants_artifact", _boom)
    out = _mgr()._hide_artifact_tool_without_request(_surface(), "anything")
    assert "create_artifact" in out
