...
from jarvis.clis.capability_provider import PLUGIN_CLI_OVERLAP, equivalent_grants

_ROOT = Path(__file__).resolve().parents[3]


def _plugin_ids() -> set[str]:
    data = json.loads((_ROOT / "jarvis/marketplace/seed_catalog.json").read_text(encoding="utf-8"))
    return {p["id"] for p in data.get("plugins", [])}


def _cli_names() -> set[str]:
    data = json.loads((_ROOT / "jarvis/clis/catalog/seed_catalog.json").read_text(encoding="utf-8"))
...
def test_equivalent_grants_cover_plugin_and_cli_spellings():
    assert equivalent_grants(("gmail",)) == ("gmail", "cli_gws")
    assert equivalent_grants(("plugin:gmail",)) == ("plugin:gmail", "cli:gws")
    assert equivalent_grants(("github", "search_web")) == ("github", "cli_gh", "search_web")
    assert equivalent_grants(("google_calendar", "google_drive")) == (
        "google_calendar",
        "cli_gws",
        "google_drive",
    )
    assert equivalent_grants(("cli_gws",)) == ("cli_gws",)
    assert equivalent_grants(()) == ()
