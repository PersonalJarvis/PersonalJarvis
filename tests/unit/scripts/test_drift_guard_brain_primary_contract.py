"""Contract test: ``scripts/config-soll.json`` ``brain.primary`` <-> ENV mapping.
...
* ``scripts/config-soll.json``   -> ``brain.primary``  (the drift-guard's soll)
...
# scripts/config-soll.json lives at <repo>/scripts/config-soll.json.
# This test file is at <repo>/tests/unit/scripts/, so parents[3] is the repo root.
CONFIG_SOLL_PATH = (
    Path(__file__).resolve().parents[3] / "scripts" / "config-soll.json"
)
...
def soll() -> dict:
    assert CONFIG_SOLL_PATH.is_file(), (
        f"expected the drift-guard soll file at {CONFIG_SOLL_PATH}"
    )
    return json.loads(CONFIG_SOLL_PATH.read_text(encoding="utf-8"))
...
def test_config_soll_is_valid_json(soll: dict) -> None:
    """If the JSON is malformed the whole drift-guard fails to parse (exit 4)."""
    assert isinstance(soll, dict)


def test_brain_section_exists(soll: dict) -> None:
    assert "brain" in soll, "config-soll.json must keep a top-level 'brain' section"
    assert isinstance(soll["brain"], dict)


def test_brain_primary_is_non_empty_string(soll: dict) -> None:
    ...
    switch would have no soll to keep in sync -> silent revert risk.
    """
    primary = soll["brain"].get("primary")
    assert primary is not None, "brain.primary must be present in config-soll.json"
...
def test_brain_primary_env_name_is_canonical(soll: dict) -> None:
    ...
    maintainer sets by hand. If the section/key in config-soll.json is ever
    ...
    """
    assert "primary" in soll["brain"]
    env_name = _derive_env_name("brain", "primary")
    assert env_name == "JARVIS__BRAIN__PRIMARY"


def test_env_derivation_rule_matches_guard_for_all_keys(soll: dict) -> None:
    """Every (section, key) in the soll maps to a JARVIS__-prefixed ENV name.
    ...
    """
    for section, payload in soll.items():
