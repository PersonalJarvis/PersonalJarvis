...
#
# Failure fixture: "Open Discord and then go on the ExampleHub
...
_DISCORD_GOAL = (
    "open discord and then go on the examplehub discord and "
...
_DISCORD_DESKTOP_PLAN = (
    ...
    '{"intent": "open the examplehub server", '
    '"success": "the examplehub server is selected"},'
...
async def test_desktop_app_goal_plans_open_app_not_browser() -> None:
    ...
    brain = PlanningBrain(
        ...
        judge_script=['{"done": true, "proof": "examplehub newest post visible"}'],
    )
    ctx = make_ctx(brain, verify=True)
