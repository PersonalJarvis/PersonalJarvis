...
persist across a restart. ``config-soll.json`` pins the whole ``tts`` block
...
3-layer. The TTS switch must therefore write ALL THREE layers:
  ...
  2. ``scripts/config-soll.json`` ``tts.*`` (drift-guard soll values)
...
def sample_soll(tmp_path: Path) -> Path:
    """A config-soll.json skeleton that pins the whole tts block."""
...
def test_set_tts_provider_writes_all_three_layers(
    ...
    # Layer 2: config-soll.json tts.provider.
    soll = json.loads(sample_soll.read_text(encoding="utf-8"))
    assert soll["tts"]["provider"] == "grok-voice"

    # Layer 3: ENV setter called with the canonical var name + value.
    assert env_calls == [("JARVIS__TTS__PROVIDER", "grok-voice")]


def test_config_soll_mirrors_provider_dependent_voice(
    sample_toml: Path, sample_soll: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    """The drift-soll must mirror the SAME voice the TOML write applied.

    Switching gemini -> grok-voice rewrites voice_de/voice_en to a grok voice
    in the TOML. If config-soll kept the old "Charon" pin, the drift-guard
    would revert the voice to a value invalid for grok. So config-soll must end
    ...
    # Whatever voice the TOML write picked, config-soll agrees with it.
    assert 'voice_de = "leo"' in toml_raw
    assert soll["tts"]["voice_de"] == "leo"
    assert soll["tts"]["voice_en"] == "leo"


def test_config_soll_mirrors_preserved_valid_voice(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    """A valid custom voice that the TOML write PRESERVES must still be mirrored
    into config-soll — otherwise the drift-guard reverts the user's choice.
    ...
    in the TOML (user override wins). But config-soll still pins "Charon" — so
    the guard would revert "Orus" -> "Charon" within 5 minutes. config-soll must
    ...
    # ...and config-soll now agrees with it (no Charon revert).
...
def test_pluginless_provider_does_not_blank_voice(
    ...
    monkeypatch.setattr(config_writer, "_config_soll_path", lambda: tmp_path / "nope.json")
    monkeypatch.setattr(config_writer, "_set_user_env_var", lambda name, value: None)
...
def test_config_soll_sync_preserves_other_keys(
    ...
    assert soll["tts"]["fallback"] == "grok-voice"
...
def test_raises_on_missing_toml(
    ...
    with pytest.raises(FileNotFoundError):
        config_writer.set_tts_provider("grok-voice", path=tmp_path / "nope.toml")
...
def test_set_tts_provider_cartesia_writes_all_layers_and_preserves_subtable(
    ...
    leaves the pre-existing [tts.cartesia] subtable and config-soll _comment
    untouched.
    ...
    config-soll (so voice_de/voice_en stayed pinned to "Charon" from the old
...
""",
    ...
    # config-soll with a _comment and a tts.cartesia sub-table that must be
    # preserved unchanged.
    soll = tmp_path / "config-soll.json"
    soll.write_text(
        json.dumps(
    ...
    # Layer 2: config-soll tts.provider == "cartesia".
    soll_data = json.loads(soll.read_text(encoding="utf-8"))
    assert soll_data["tts"]["provider"] == "cartesia"

    # The _comment and tts.cartesia sub-table in config-soll are untouched.
    assert soll_data["_comment"] == "do not lose me"
    assert soll_data["tts.cartesia"]["model_id"] == "sonic-3.5"
