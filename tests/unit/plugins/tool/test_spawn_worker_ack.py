...
async def test_context_bleed_utterance_falls_back_to_verbatim_turn() -> None:
    ...
    ("emigrate to Exampleville") in both the utterance and action args — the
    ...
    await tool.execute(
        {
            "utterance": "research everything I need to emigrate to Exampleville",
            "action": "prepare a detailed Exampleville emigration report",
            "target": "Exampleville emigration checklist",
        },
        ctx,
    )
    await _drain_background_tasks()
    ...
    assert "Exampleville" not in prompt
