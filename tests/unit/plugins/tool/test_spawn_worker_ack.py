...
async def test_context_bleed_utterance_falls_back_to_verbatim_turn() -> None:
    ...
    ("research renewable transit") in both the utterance and action args — the
    ...
    await tool.execute(
        {
            "utterance": "research renewable transit systems",
            "action": "prepare a detailed renewable transit report",
            "target": "renewable transit comparison",
        },
        ctx,
    )
    await _drain_background_tasks()
    ...
    assert "renewable transit" not in prompt
