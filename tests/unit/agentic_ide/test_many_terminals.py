"""Workspace capacity, independent sessions and bounded persistent metadata."""
...
async def test_closing_the_middle_of_a_large_workspace_repacks_it(
    ...
    victim = session.terminals[ide.MAX_TERMINALS // 2].name
    await registry.close_terminal(victim)

    assert len(session.terminals) == ide.MAX_TERMINALS - 1
    columns = sorted({t.column for t in session.terminals})
    assert columns == list(range(len(columns)))
    assert [t.index for t in session.terminals] == list(range(ide.MAX_TERMINALS - 1))
...
async def test_a_batch_past_the_cap_is_rejected_without_partial_creation(
    registry: ide.Registry, tmp_path: Path
) -> None:
    """An oversized batch must not silently create a smaller agent group."""
    await registry.start(str(tmp_path), [{"agent": "claude"} for _ in range(ide.MAX_TERMINALS - 3)])
    with pytest.raises(ide.SessionError, match="at most"):
        await registry.add_terminals(10)
    assert len(registry.session.terminals) == ide.MAX_TERMINALS - 3
...
async def test_restoring_a_full_workspace_keeps_every_position(
    ...
    # Every pane sits exactly where it sat — four columns of two — read
...
async def test_a_full_house_across_every_workspace_round_trips(
    ...
    for index in range(workspace_count):
        ...
        await registry.start(str(folder), [{"agent": "claude"} for _ in range(ide.MAX_TERMINALS)])

    loaded = resume_store.load()
    assert loaded is not None
    assert len(loaded.workspaces) == workspace_count
    assert loaded.terminal_count == ide.MAX_TERMINALS * workspace_count
    for workspace in loaded.workspaces:
        names = [t.name for t in workspace.terminals]
        assert names == default_names(ide.MAX_TERMINALS), "each workspace numbers its own panes"
