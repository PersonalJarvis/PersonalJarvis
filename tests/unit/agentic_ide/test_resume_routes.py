...
def _store(folder: Path, *, with_conversation: bool = True) -> None:
    resume_store.save(
        resume_store.Snapshot(
            ...
            terminals=[
                resume_store.SnapshotTerminal(
                    ...
                    resume=(
                        ResumeHandle(
                            kind="claude_session", id="u", captured_at=1.0
                        )
...
def test_the_offer_names_the_panes_and_where_they_sat(
...
def test_the_offer_survives_a_machine_with_no_coding_cli(
    ...
    body = client.get("/api/agentic-ide/resume").json()
    assert body["available"] is False
    assert all(t["available"] is False for t in body["terminals"])
...
def test_resuming_a_deleted_folder_says_so_plainly(
    client: TestClient, tmp_path: Path
) -> None:
...
def test_resuming_starts_no_agent_by_itself(
    client: TestClient, tmp_path: Path
) -> None:
    """The panes connect as they always do; that is what starts them."""
    _store(tmp_path)
    body = client.post("/api/agentic-ide/resume").json()
    assert all(t["status"] == "pending" for t in body["session"]["terminals"])


# ------------------------------------------------------------- start fresh
def test_starting_fresh_withdraws_the_offer(
    client: TestClient, tmp_path: Path
) -> None:
    _store(tmp_path)
