...
updated ``config-soll.json`` (the drift-guard's desired state) and the Python
...
files silently disagreed: soll said opus, toml said fable, and every worker
...
These tests fail loudly the moment ``jarvis.toml`` and ``config-soll.json``
...
_CONFIG_SOLL = _REPO_ROOT / "scripts" / "config-soll.json"
...
def _load_soll() -> dict:
    return json.loads(_CONFIG_SOLL.read_text(encoding="utf-8-sig"))
...
def _soll_value(soll: dict, dotted: str, key: str) -> str | None:
    block = soll.get(dotted)
...
def test_soll_worker_model_pins_are_reachable() -> None:
    ...
    config-soll.json ships in git (cloud-first), so this always runs — including
    on a fresh VPS checkout — and guards against re-pinning Fable in the soll.
    """
    soll = _load_soll()
    for dotted, key in _MODEL_KEYS:
        value = _soll_value(soll, dotted, key)
        assert value not in _UNREACHABLE_CLAUDE_MODELS, (
            f"config-soll.json {dotted}.{key} = {value!r} is approved-access-only "
...
@pytest.mark.skipif(
    ...
    reason="no local jarvis.toml (fresh checkout / headless VPS) — soll guard covers it",
)
def test_jarvis_toml_model_pins_match_soll() -> None:
    """``jarvis.toml`` (what ``load_config`` reads) must agree with the approved
    ``config-soll.json`` on every worker/critic model pin.

    This is the exact invariant that broke on 2026-06-14: soll updated to
    ...
    for dotted, key in _MODEL_KEYS:
        ...
        soll_val = _soll_value(soll, dotted, key)
        ...
        # exists to catch).
        if toml_val is None or soll_val is None:
            mismatches.append(
                f"  {dotted}.{key}: missing pin (jarvis.toml={toml_val!r}, "
                f"config-soll={soll_val!r}) — both must define it"
            )
        elif toml_val != soll_val:
            mismatches.append(
                f"  {dotted}.{key}: jarvis.toml={toml_val!r} != config-soll={soll_val!r}"
            )
