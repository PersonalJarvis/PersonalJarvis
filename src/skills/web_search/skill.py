...
the web-search capability. The risk tier is fixed (not read from
config) so that the safety classification cannot drift via runtime mutation
of a TOML — see ADR-021.
"""
...
# ``RISK_TIER``; this name is kept so external callers that already imported
# ``SKILL_RISK_TIER`` keep working. See ADR-021 §Decision.
