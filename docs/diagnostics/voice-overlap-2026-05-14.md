...
| Same TTS provider is called twice for the same text | **Ruled out** | The Announcement text ("Ich prüfe den Status der Spotify-App …") and the brain answer come from different LLM endpoints (Gemini Flash Lite for ack, Grok/Gemini for main brain). |
