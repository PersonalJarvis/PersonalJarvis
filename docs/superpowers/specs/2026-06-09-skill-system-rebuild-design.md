...
`BrainManager._should_force_spawn` (`jarvis/brain/manager.py:2785-2798`) evaluates *before* the brain ever sees the system prompt. Action-verb utterances ("starte die Morgenroutine") match the spawn heuristic / `_is_generic_subagent_work` and are dispatched to an OpenClaw mission. The brain — and therefore the `run-skill` tool — never gets a chance. Non-plugin builtins (morning-routine, deep-work-mode) register no capability at all, so the capability registry classifies them as "action with no capability" → generic subagent work → force-spawn.
