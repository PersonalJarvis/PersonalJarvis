...
**Assistant modes (`jarvis/brain/modes.py`):** the character the assistant answers in — five built-ins (`jarvis/brain/modes/*.md`, packaged) plus the user's own under `DATA_DIR/modes/`. A mode is **layered onto** the base persona, never substituted for it, so no mode can drop the standing honesty rules; the default `assistant` mode contributes an empty block, making the feature invisible until it is used. Because `load_effective_persona_prompt()` is the single chokepoint both the typing brain (`BrainManager._build_system_prompt`) and the speaking brain (`jarvis/realtime/session.py`) read fresh each turn, one switch reaches voice and chat together with no restart. Which mode is active has two layers: `[persona] active_mode` in `jarvis.toml` (the user's sticky choice, written only via `config_writer.set_active_mode`) and an **in-memory section override** the Agentic IDE sets while it is on screen — never persisted, so a mode a screen turned on cannot outlive the process. Surfaces: `/api/modes`, `jarvis modes …`, the Modes section, and the `list_modes` / `switch_mode` / `save_mode` router tools (ADR-0011 amendment "Assistant-Mode Tools").
