...
- **One secondary, non-video finding:** a stray ShareX capture
  ...
  `ruben.luetke10@gmail.com`** twice. It is `.gitignore`d and is **NOT** used in
...
(`reporttutorialpickupprompt (1).md`), so it is Ruben's informal label, not a
filename.
...
The *finished film* is meant to be **public on YouTube** — Ruben uploads it himself
...
the voiceover names "Ruben … a developer from Germany", and the wake word is
"Hey Ruben". One stray (non-video) screenshot also embeds the private email. No
...
**Personal email(s)**
- `ruben.luetke10@gmail.com`: **only in pixels** of
...
**GitHub login**
- `rubenluetke10-beep`: in pixels of `shot-github.png` (commit-author line +
...
- `video/public/shot-wake-crop.png` — "Hey Ruben" / "called: Ruben" (first name).
...
- `video/public/personal-jarvis-onboarding-thumbnail.png` (public YouTube
  thumbnail) — only the first name in the "Hey Ruben …" command cards; no email,
...
- `https://discord.gg/6VzzNDwUwV` in `video/README.md:46` is **Remotion's official
  template Discord** (boilerplate from the starter), **not** Ruben's server — not
  sensitive.
...
Ruben actively rehearsing the voiceover ("In this short onboarding video I'll show
...
3. **Everything else is fine to ship as-is.** First name "Ruben" / "Hey Ruben",
