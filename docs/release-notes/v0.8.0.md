...
**Plan:** `ReviewSubagent/Unbenanntes Dokument (5).md` (== JARVIS_REVIEW_AGENT_PLAN.md)

## Summary

Personal Jarvis gets a **quality-gate pipeline** that has the outputs of
the sub-Jarvis worker validated by a
...
- **AD-14** (Phase 8.7): voice-phrase wording kept constant
  ("Lass mich kurz an der Aufgabe arbeiten." / "Erledigt — {summary}" /
  "Mein bestes Ergebnis liegt vor, mit einer Einschränkung: {top_issue}").
