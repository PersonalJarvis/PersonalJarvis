...
| P-17 | Low | CU verify | Popup/menu-surface enumeration (`visible_popup_windows` / `open_menu_surface_present`) is Windows-only (EnumWindows + class/style probe); it is positive-only effect EVIDENCE — off Windows the effect check simply proceeds without it (menus there render inside the app window/layer and stay visible to the pixel diff) | `jarvis/platform/window_state.py::visible_popup_windows` | macOS/Linux: empty result, no behavior change vs pre-2026-07-21 |
| P-18 | Low | CU verify | Depth-free focused-element probe (`query_focused_element`) has Windows (UIA `GetFocusedElement`) and macOS (`AXFocusedUIElement`, unverified-on-real-desktop) backends; Linux/AT-SPI has no cheap global focus query yet | `jarvis/vision/element_at_point.py::query_focused_element` | Linux: probe returns `None`; typed-text/click verification keeps the walked-tree verdict (pre-2026-07-21 behavior) |
