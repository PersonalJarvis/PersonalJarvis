...
def _kill_process_group() -> None:
    try:
        ...
        # If group signaling is unavailable, exiting this supervisor still
        # closes its descriptors and lets the OS reap the contained process.
        os._exit(137)
...
def _watch_parent(lifeline_fd: int, completed: threading.Event) -> None:
    try:
        ...
        # A read error is equivalent to a closed parent lifeline; the kill
        # decision immediately below remains the observable handling.
        pass
...
def run(lifeline_fd: int, command: Sequence[str]) -> int:
    ...
    try:
        ...
        print(f"Could not start contained child process: {exc}", file=sys.stderr)
        return 127
    finally:
        completed.set()
        try:
            ...
            # Best-effort descriptor cleanup after the child has terminated.
            pass
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        # Invalid internal CLI input is reported through exit status 2.
        return 2
