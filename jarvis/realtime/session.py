...
class RealtimeVoiceSession:
        ...
        try:
            async for event in self._session.receive():
                if event.type == "input_transcript":
                    ...
                    if self._recover_empty_provider_turn():
                        continue
    ...
    def _recover_empty_provider_turn(self) -> bool:
        ...
        Turns with tool evidence are deliberately excluded because replaying
        the original request could repeat a side effect. Delegate-owned turns
        already have their own result lifecycle and are likewise never retried.
        """
        turn_id = self._turn_id
        if (
            not turn_id
            or not self._last_user_text
