...
- ``plain`` — Reine Textversion ohne Markdown-Marker, fuer Copy in
  Voice-Memo/E-Mail/Plain-Text-Editoren. Keine Emojis, statt dessen
  ASCII-Praefixe (``[USER]``, ``[JARVIS]``, ``[TOOL]``).
...
def format_session_plain(
    ...
    """ASCII-only-Variante ohne Markdown-Marker und ohne Emojis."""
    turns_list = list(turns)
    lines: list[str] = []
    ...
    lines.append(f"Sprache:  {session.language}")
    if session.hangup_reason:
        lines.append(f"Ende:     {_pretty_hangup(session.hangup_reason)}")
    if session.total_cost_usd > 0:
        lines.append(f"Kosten:   ${session.total_cost_usd:.4f}")
    lines.append("")
    lines.append("=" * 60)
    ...
    for t in turns_list:
        lines.append(
            f"--- Turn {t.idx + 1} ({_fmt_dt(t.started_ms, time_only=True)}) ---"
        )
        if t.user_text:
        ...
        if t.model:
            meta_parts.append(f"model={t.model}")
        if t.tokens_in or t.tokens_out:
            meta_parts.append(f"tokens={t.tokens_in}+{t.tokens_out}")
        if t.cost_usd > 0:
            meta_parts.append(f"cost=${t.cost_usd:.4f}")
        if t.latency_total_ms > 0:
            meta_parts.append(f"latency={_fmt_ms(t.latency_total_ms)}")
        if t.think_ms > 0:
            meta_parts.append(f"think={_fmt_ms(t.think_ms)}")
        if t.speak_ms > 0:
            meta_parts.append(f"speak={_fmt_ms(t.speak_ms)}")
        if meta_parts:
            lines.append("[BRAIN]  " + " ".join(meta_parts))
        if t.tool_calls:
            lines.append("[TOOLS]  " + ", ".join(t.tool_calls))
        if t.jarvis_text:
            lines.append(f"[JARVIS] {t.jarvis_text}")
        lines.append("")
