...
class InterruptedPane:
    ...
    display_name: str
    status: str
    #: Can it be nudged right now? False for a pane whose agent is not running —
    #: an instruction cannot be typed into a dead terminal, and saying so before
...
class ContinueReport:
    ...
    Three lists rather than a count, because the three outcomes need different
    words from whoever reports this: a pane that took the instruction, one that
    refused it, and one that took the text but never submitted it (the prompt is
    sitting in its input box and a human has to press Enter). Collapsing the
    third into either of the others is the lie :meth:`Registry.send_prompt` goes
...
def _blocked_reason(term: Terminal) -> str:
    ...
    if term.status == "pending":
        return "Its agent has not started yet — open the workspace so the pane connects."
...
async def continue_panes(
    ...
    *,
    ...
    blocked = [pane for pane in chosen if not pane.continuable]
    failed.extend((pane.name, pane.blocked_reason) for pane in blocked)
    runnable = [pane for pane in chosen if pane.continuable]
    ...
    for pane, outcome in await asyncio.gather(*(_send(pane) for pane in runnable)):
        if isinstance(outcome, Exception):
            ...
            # keypress in a pane they can see.
            unconfirmed.append(pane.name)

    if continued or unconfirmed or failed:
        logger.info(
            "Agentic IDE: continued {} interrupted pane(s) ({} unconfirmed, {} refused)",
            len(continued),
            len(unconfirmed),
            len(failed),
        )
    return ContinueReport(continued=continued, unconfirmed=unconfirmed, failed=failed)
