...
openWakeWord is a free, open-source wake-word system (MIT), uses
ONNX Runtime, and has "hey_jarvis" built in as a pretrained model —
zero API key needed.

Input format: 16 kHz mono int16 PCM, frame size must be 1280 samples (80 ms).
We buffer mic chunks in and split on the frame boundary.
...
class OpenWakeWordProvider:
    ...
    By default it listens for "hey_jarvis" — from my testing, the pretrained
    model responds to both "Hey Jarvis" and to "Jarvis" alone, when the
    ...
    def __init__(
        ...
        # so scores are visible at all while testing (otherwise silent when the
        # user speaks too quietly or the model doesn't match German pronunciation well).
    ...
    def _ensure_model(self) -> None:
        if self._model is None:
            ...
            # wakeword_models=[...] accepts either built-in names or
            # paths to .onnx / .tflite files. We prefer the bundled
            # local ONNX paths (see _model_kwargs) — no runtime download,
            # offline-capable on first start.
            self._model = Model(**self._model_kwargs())

    async def start(self) -> None:
        """Pre-load the model — saves latency on the first audio frame."""
    ...
    async def detect(
        ...
        """Consumes audio chunks, yielding keyword names on detection.
        ...
        async for chunk in chunks:
            # Int16 view onto the bytes — openWakeWord expects int16 arrays
            ...
            for frame in frames:
                ...
                for keyword, score in scores.items():
                    # Live debugging: log everything over `score_log_threshold`,
                    # so the user can see during testing how close they are to a hit.
