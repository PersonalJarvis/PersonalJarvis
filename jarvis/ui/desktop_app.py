...
class DesktopApp:
    ...
    def window_chrome_snapshot(self) -> dict[str, Any]:
        """What the page should draw in the title-bar corner.

        ``frameless`` follows the live window. A shell that has not opened one
        yet reports no buttons, so a page never shows controls that cannot work.
        """
        from jarvis.ui.window_command import window_chrome

        info = window_chrome()
        window = self._window
        frameless = bool(getattr(window, "frameless", False)) if window is not None else False
        info["frameless"] = frameless
        if not frameless:
            info["controls"] = "none"
        return info
    ...
    def window_command(self, action: str, view: str | None) -> dict[str, Any]:
        ...
        if sys.platform == "win32" and window is not None:
            try:
                from jarvis.ui.window_frame import native_hwnd, window_is_maximized

                hwnd = native_hwnd(window)
                if hwnd:
                    maximized = window_is_maximized(hwnd)
            except Exception:  # noqa: BLE001
                from loguru import logger
    ...
    def _arm_window_frame(self, window: Any) -> None:
        ...
        Other platforms keep the resizable window mask pywebview already sets.
        A failure here leaves the window usable: the page's own buttons still
        minimize, maximize and close it. Logged, not swallowed silently.
        """
        if sys.platform != "win32" or window is None:
            return

        def _install(*_args: Any, **_kwargs: Any) -> None:
            try:
                from jarvis.ui.window_frame import install_resize_frame, native_hwnd
                ...
                logger.exception("The window edge resize was not installed")

        try:
            window.events.shown += _install
        except Exception:  # noqa: BLE001
            from loguru import logger

            logger.exception("Could not subscribe the window frame hook")
        _install()
    ...
    def _hook_main_window_lifecycle(self) -> None:
        ...
        self._arm_window_frame(self._window)
