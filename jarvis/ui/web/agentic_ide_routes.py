...
* ``PATCH  /terminals/{name}``           → give that pane another call-sign
...
@router.patch("/terminals/{name}", summary="Rename one terminal")
async def rename_terminal(name: str, req: RenameTerminalRequest) -> dict:
    """Give the pane ``name`` another call-sign.
    ...
    """
    try:
        session, term = await get_registry().rename_terminal(name, req.name)
    except SessionError as exc:
        message = str(exc)
        status = 409 if "already called" in message else 404
        raise HTTPException(status_code=status, detail=message) from exc
    return {
        "ok": True,
        "renamed": term.name,
