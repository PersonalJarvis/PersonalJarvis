...
async def get_voice_mode(request: Request) -> dict[str, object]:
    ...
    browser_audio = bool(cfg is not None and await asyncio.to_thread(realtime_browser_audio, cfg))
    transport_offer_ready = (
        True if browser_audio else await _realtime_transport_offer_ready(requires_webrtc_offer)
    )
