...
``POST /save`` is **desktop-only**: it is gated on
``app.state.native_file_actions`` (True on a local desktop run, False on a
headless VPS) and 404s when False — on a server the file would land on the
*server's* disk, not the user's machine, so the frontend keeps the browser
