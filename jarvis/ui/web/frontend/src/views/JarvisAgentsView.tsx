...
export function JarvisAgentsView({ onSelectAgent, onSelectPlace, onOpenAgents, onMarsSelectionChange }: JarvisAgentsViewProps) {
  ...
  const [mars, setMars] = useState(() => new URLSearchParams(window.location.search).get("world") === "mars");
  ...
  const switcher = <button type="button" onClick={switchWorld} data-mars-ui
