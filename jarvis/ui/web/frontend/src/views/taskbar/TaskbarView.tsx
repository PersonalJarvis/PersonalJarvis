...
import { Monitor, Eye, Volume2, PanelBottom } from "lucide-react";
...
/**
 * Taskbar section — the on-screen overlay appearance (Bar / Mascot / None) and
 * two dictation behaviours: show the bar at all times, and mute music while a
 * voice session is active. Brand cards (charcoal + gold), Wispr-style rows.
 */
export function TaskbarView() {
  ...
  return (
    <div className="flex h-full flex-col">
      <ViewHeader
        icon={<PanelBottom className="h-4 w-4 text-primary" />}
        title={t("nav.taskbar")}
        subtitle={t("taskbar_view.subtitle")}
      />
...
/**
 * On-screen overlay style selector (Bar / Mascot / None). Moved here from the
 * Settings view — single source. Reuses the settings_view.overlay_style.* i18n.
 */
function OverlayStylePanel() {
  ...
  // A bar <-> mascot switch cannot apply live (BUG-031: Tcl cross-thread abort),
  // so the app self-restarts to deliver it — no manual close + reopen. The
  // window goes away on success, so we never clear ``restarting`` there.
