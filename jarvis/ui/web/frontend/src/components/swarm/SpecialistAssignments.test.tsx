...
describe("explicit team-scoped specialist assignments", () => {
  it("loads on demand and only projects active source identity fields", async () => {
    ...
    const picker = await screen.findByRole("combobox", { name: "Choose an active specialist" });
  ...
  it("transfers only explicit input and revokes the team identity without mutating its source", async () => {
    ...
    fireEvent.click(screen.getByText("Specialist assignments", { selector: "summary" }));
    const form = await screen.findByRole("form", { name: "Assign to this team" });
    const picker = within(form).getByRole("combobox", { name: "Choose an active specialist" });
    await waitFor(() => expect(picker.hasAttribute("disabled")).toBe(false));
    expect(within(form).getByRole("button", { name: "Assign to this team" }).hasAttribute("disabled")).toBe(true);
    fireEvent.keyDown(picker, { key: "ArrowDown" });
    const list = await screen.findByRole("listbox", { name: "Choose an active specialist" });
    fireEvent.keyDown(list, { key: "ArrowDown" });
    fireEvent.keyDown(list, { key: "Enter" });
    expect(picker.getAttribute("data-value")).toBe("researcher");
