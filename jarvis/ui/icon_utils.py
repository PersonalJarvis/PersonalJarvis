...
def set_window_icon_for_current_process(ico_path: Path) -> bool:
    """Set the icon on THIS process's top-level visible window(s).

    More robust than matching by title: WebView2 rewrites the host window's
    title to the page's ``document.title`` (so the title flips from
    "Personal Jarvis" to the React app's title once it loads), which made
    ``FindWindowW(None, "Personal Jarvis")`` miss the window and leave the
    taskbar on the generic Python icon. Enumerating our own PID's top-level
    titled windows is title-independent and hits the real taskbar window.

    Returns True if the icon was applied to at least one window.
    """
    if sys.platform != "win32":
        return False
    if not ico_path.is_file():
    ...
    def _cb(hwnd: int, _lparam: int) -> bool:
        ...
        if pid.value == our_pid and user32.IsWindowVisible(hwnd):
            # Only top-level windows that carry a title bar (the taskbar one),
            # not borderless helper windows (overlay/tray).
