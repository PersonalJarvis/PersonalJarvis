...
def load_jarvisbar_position(path: str | Path) -> tuple[int, int] | None:
    ...
    try:
        ...
        return None
    # Backwards-compat: the position used to live under the [whisperbar]
    # section (renamed to [jarvisbar] to drop a trademarked name). Prefer the
    # new section; fall back to the legacy one so an existing jarvis.toml keeps
    # the saved position. The next save() migrates it to [jarvisbar].
