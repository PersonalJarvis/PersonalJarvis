...
#: works — not a way to silence the gate.
...
def main() -> int:
    ...
    parser.add_argument(
        "--report",
        action="store_true",
        help="list the findings but always exit 0 (for triage, not for CI)",
    )
    args = parser.parse_args()

    dead = unread_fields()
    if not dead:
        print(f"OK: all {len(config_fields())} config fields are read by shipped code.")
        return 0

    print(f"Dead config switches: {len(dead)} field(s) no code reads.\n")
