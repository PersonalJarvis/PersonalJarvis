...
for (const { id, file: name, poster } of settings.compositions) {
  ...
  ffmpeg(["-i", master, "-i", palette, "-lavfi", `${filter}[scaled];[scaled][1:v]paletteuse=dither=none:diff_mode=rectangle`, "-loop", "0", path.join(destination, `${name}.gif`)]);
