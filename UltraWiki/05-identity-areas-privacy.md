...
2. **Probable — queue for confirmation.** Name similarity, nickname patterns,
   co-occurrence in the same threads ("Test" appears exactly where Testoria's
   ...
   **confirmation queue** and both identities keep answering separately until
   the user confirms ("Is Test = Testoria Example?" — one tap). <!-- i18n-allow: illustrative German name in the identity-merge example -->
