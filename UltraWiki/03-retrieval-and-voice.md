...
- **Planner.** Extracts structured slots from the question and picks which
  retrieval lists matter ("Testoria" → entity lookup; "when" → the answer is a
...
the calendar says "Dinner w/ Testoria", the photo's metadata says Exampleville. The event extraction on the write path (doc 02) has already fused
...
1. **Precomputed profiles.** Every entity keeps a stored, continuously
   re-summarized profile ("who is Testoria", "what is Project X"), refreshed on
