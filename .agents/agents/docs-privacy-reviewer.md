...
**(B) Maintainer identity** — real name in any spelling/casing (`Alex`/`Alex`/`Maintainer`/`Maintainer`), alt name (`Sam`/`Sam Herz`), personal GitHub login (`octocat`), private emails (`alex.maintainer10@gmail.com`, `sam.herz@gmx.de`), `owner: <personal-name>` frontmatter.  <!-- i18n-allow -->

**(C) Personal filesystem / machine identifiers** — `<USER_HOME>\…`, personal OneDrive paths, the `C--Users-Administrator-…` memory-dir slug, Windows SID (`S-1-5-21-…`), machine/account name, internal GCP/project ids.
...
- Product/brand names (Jarvis, Claude, Anthropic, OpenAI, Gemini), the public repo `PersonalJarvis`, the public Discord invite, the **intentionally public** X handle `@Alex_Herz` (an explicit author-credit decision — see `scrub-exempt.txt`).
...
1. **`<path>:<line>`** — <category A/B/C/D> — <what it is>
   **Mask:** <the canonical replacement, e.g. Alex→Alex, the path→<USER_HOME>, the secret→<REDACTED>>
