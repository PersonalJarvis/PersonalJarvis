...
1. `docs/agent-contract.md` §7 — the anti-pattern register **AP-1..AP-31**. This
   is the only live register. (`AGENTS.md` is a byte-identical twin of
   `CLAUDE.md`, not a second document — reading both is reading one.)
2. `CLAUDE.md` — the compressed index of the same contract.
3. `docs/BUGS.md` §8 bug classes — what these anti-patterns cost in production.
4. The changed files themselves, read in full, not just the diff.
...
found, conforms to the contract in docs/agent-contract.md` — and return
