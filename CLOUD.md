...
1. **The maintainer's "push / commit / save to GitHub / sichere den Stand" goes to the PUBLIC flagship `PersonalJarvis/PersonalJarvis` via the `ship-public-release` skill** — a clean depersonalized snapshot through the full privacy gate (incl. the mandatory sub-agent review), NOT a raw `git push origin`. The lowercase `origin` (`personal-jarvis`) is kept ONLY as a silent local backup / dev safety net; automatic raw dev commits may land there as infrastructure, but that is never what the maintainer means by "push". Never ask "which repo" — the deliverable is always the public flagship. <!-- i18n-allow: quoted German maintainer trigger phrase -->
