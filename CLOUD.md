...
4. **No new dependency that can break the base install or the boot.** The test is behavioural, not a name list: `pip install` and `import jarvis` must succeed on a fresh `python:3.11-slim` with no GPU, no audio device, and no native OS API. Three mechanisms satisfy it, and a package may use whichever fits: an **extras group** (`[desktop]`, `[desktop-linux]`, `[desktop-macos]`, `[local-voice]`), an **environment marker** that keeps the wheel off the platforms it cannot serve, or a **lazy import behind a capability probe** so the module never loads where it would fail. `pywin32`, `pywinauto`, `pyautogui`, `faster-whisper`, `onnxruntime-gpu`, `global-hotkeys`, `mss` and `pywebview` are extras. `sounddevice`, `openwakeword`, `onnxruntime`, `vosk` and `aiortc` sit in the base install *by marker and lazy import* — they install cleanly, and the features that need them degrade honestly when the system library or device is absent. Gates: `check_import_clean.py` (module-scope platform imports), `check_requirements_sync.py` and `check_lockfile_universal.py` (torch-free, universal lock), `check_portable_install_matrix.py` (the install itself). Adding a package that fails any of those is the defect — the wording here follows the gates, not a frozen list of names.
