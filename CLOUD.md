...
| `public` | `github.com/PersonalJarvis/PersonalJarvis` — PascalCase repo name | The **depersonalized public distribution repo** (still private until the public launch, ~end of June). A clean, secrets-/PII-scrubbed snapshot of the working tree. | **Only** the depersonalized public-release process (privacy gate under `scripts/ci/privacy_gate/`). |

**Binding rule for every agent (human or cloud-code):**

1. **The maintainer's "push / commit / save to GitHub / sichere den Stand" goes to the PUBLIC flagship `PersonalJarvis/PersonalJarvis` via the depersonalized public-release process** — a clean depersonalized snapshot through the full privacy gate (incl. the mandatory sub-agent review), NOT a raw `git push origin`. The lowercase `origin` (`personal-jarvis`) is kept ONLY as a silent local backup / dev safety net; automatic raw dev commits may land there as infrastructure, but that is never what the maintainer means by "push". Never ask "which repo" — the deliverable is always the public flagship. <!-- i18n-allow: quoted German maintainer trigger phrase -->
2. **`PersonalJarvis/PersonalJarvis` (the `public` remote) is written EXCLUSIVELY by the depersonalized public-release process** — never by a manual `git push`, never with raw working state, never with secrets/PII. Pushing day-to-day work there is a defect.
...
5. **Push "volume" — one public repo, two ceremonies (UPDATED 2026-06-19; supersedes the 2026-06-09/15 rules).** Everything reaches `PersonalJarvis/PersonalJarvis` through the depersonalized public-release fail-closed privacy gate: tracked-files export → distribution denylist → deterministic PII scrub → **mandatory sub-agent privacy review** → deterministic secret/PII scan → human review. Raw working state NEVER goes there (the pre-push guard hard-blocks it; the skill pushes from a separate clean clone). Two ceremonies on the SAME gate: a **DISCREET** snapshot (DEFAULT — "push", "Push nach GitHub", "commit and push", "sichere den Stand"; a clean snapshot, **no version bump, no git tag, no GitHub Release, no announcement**) and an explicit **RELEASE** ("Neue Version shippen", "Mach ein Release", "veröffentliche eine neue Version"; the same gate **plus** a MAJOR/MINOR/PATCH bump + tag + CHANGELOG). When the volume is ambiguous, default to **discreet**. The release process NEVER flips repo visibility — that stays the maintainer's manual call. Full contract: `CLAUDE.md` § "THE GitHub repository". <!-- i18n-allow: quoted German maintainer trigger phrases -->
