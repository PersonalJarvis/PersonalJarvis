...
**Subscription-OAuth is allowed ONLY where the vendor sanctions it; Antigravity / Gemini-*subscription* login is FORBIDDEN (BINDING, AP-21).** Billing a model through a consumer subscription instead of an API key is fine where the vendor offers an official path — Claude Max and ChatGPT/Codex expose a sanctioned CLI OAuth login, which is how Workers already run (`claude-cli`). Google does **not**: a Google AI Pro/Ultra plan has no API path, and the only way to tap it from outside Google's own tools (the **Antigravity CLI**, the Gemini-CLI OAuth token) is to impersonate the official client against Google's active abuse detection. That is detection-evasion — Google bans the account for it (the Feb 2026 mass-suspensions hit paying Ultra subscribers), it takes Gmail/Calendar/the paid plan down with it, and such code cannot be honestly verified without performing the very login that triggers the ban. **Gemini is reached ONLY via an API key** (the `gemini` provider). Never add an Antigravity/Gemini-subscription brain provider, harness, or "login via subscription" control. Enforced fail-closed by `scripts/ci/check_no_antigravity.py` (the `no-antigravity` CI gate) — it fails the build on any `antigravity` reference in scanned code/config/frontend.
...
| AP-21 | Wire an Antigravity CLI / Gemini-*subscription* OAuth provider, harness, or "login via subscription" control | Account ban: tapping the Google AI Pro/Ultra plan from outside Google's own tools is detection-evasion against Google's active abuse enforcement → Google suspends the account (Gmail/Calendar/paid plan), and it can't be honestly verified without the very login that triggers the ban. Gemini is API-key only. Blocked fail-closed by `scripts/ci/check_no_antigravity.py` (`no-antigravity` CI gate) |
