...
- **Secrets** only via `jarvis.core.config.get_secret`: OS keyring → ENV → `.env` → local-file fallback. Never put keys in code / `jarvis.toml` / commits / `.claude/`. **Installer signing PRIVATE keys live ONLY in GitHub Actions secrets — never in the repo, not even encrypted; only the PUBLIC keys ship (AP-29).** Voice/chat must never accept secrets (AP-2).
...
| AP-26 | Put a new feature's init/import on the startup critical path (before `APP_INTERACTIVE` / `VOICE_USABLE`) — a synchronous load in `_run_backend`, the `WebServer` ctor, `_start_speech_and_orb`, or a module-level heavy import | Boot creep: every feature "only adds a second" until the app takes 30+ s to become usable (TTU forensic 2026-07-02: one wake change put a 114.7 s load cascade on boot). New subsystems hook into `_heavy_backend_bg`, a deferred registry scan, or a post-ready task; heavy imports stay lazy or get a `warmup_prefetch` daemon thread; routes answer a graceful 503/None while their subsystem warms. **Enforced fail-closed by the pre-push BOOT BUDGET gate** (`scripts/ci/check_boot_budget.py`: one real cold boot; window ≤ 8 s, voice-usable + app-interactive ≤ 20 s). Doctrine + how-to: `docs/diagnostics/BOOT-TTU-NOTES.md` |
| AP-27 | Suppress `stt_match` wake ghosts by tightening TRANSCRIPT CONTENT — require the bias-echo confirm's unbiased pass to reproduce the wake word, raise the phrase-matcher strictness on the confirm, or add any second content check that must contain the phrase | Kills recognition ENTIRELY, so "fires on silence" flips into "never fires". A bias-primed local Whisper (`initial_prompt=<phrase>`, needed for proper-noun recall) HALLUCINATES the phrase on silence AND GARBLES it on real speech (`'Mythos'`→`'Mütos'`/`'Hey, Mut!'`, `'Fable'`→`'Farbe'`); the unprimed base model literally cannot spell a hard custom wake — that is the whole reason the bias exists — so NO transcript-content rule separates ghost from wake without rejecting every genuine wake (live 2026-07-02: an unbiased-corroboration check made the wake "stop working at all"). The ONLY word-agnostic discriminator is raw audio ENERGY. Fix: suppress silence at the match site with the RMS gate (`RollingWhisperWake._match_min_rms`, ghost cluster ≤ 0.0043 vs quiet-mic recall 0.009); keep the bias-echo confirm PERMISSIVE ("heard any real speech → genuine", fail-open) and SKIP it on a clearly-loud window (`_ECHO_CONFIRM_SKIP_RMS`, latency). Recall guard: `tests/unit/speech/test_rolling_whisper_wake_silence_ghost.py::test_loud_wake_fires_even_when_unbiased_pass_garbles_the_hard_word` | <!-- i18n-allow: forensic quotes of the German STT-garble tokens under test -->
| AP-28 | Gate a pre-push / CI check on `isinstance` against the *installed* copy of a third-party lib that CI installs **unpinned** (newest), then trust "green on my machine" | Version-skew brick: the lib's next release changes internals and the gate CRASHES or false-fails in CI while staying green on your older local version. Typer 0.26 vendored its own Click (`typer._click`), so `typer.main.get_command`'s root group + its params stopped being `isinstance` of the *external* `click.Group`/`Argument`/`Option` → the CLI-reference drift gate `AssertionError`'d on every push though it was green on local Typer 0.21 (fix 621f837a). Discriminate by CAPABILITY, never concrete type (`.commands`, Click's stable `param_type_name`). **Before anything goes PUBLIC (a push to the flagship, or the visibility flip): CI must be GREEN and every red run's cause understood + fixed — a red gate is a defect to explain, never "just noise". Reproduce the failure against the SAME unpinned versions CI installs (a throwaway venv), not your local pins.** |
| AP-29 | Commit a signing PRIVATE key — or its passphrase, or a `*.key.enc` "encrypted at rest" copy — into the repo | The installer is signed on key-bound axes (Wave 2 Ed25519, Wave 4 ML-DSA-65). The former scheme stored the encrypted private keys in-repo plus a DEMO passphrase in plaintext docs; that passphrase leaked into 14 public snapshots (v0.1.0..v0.9.1) — permanent, world-readable — so any key it wrapped is forgeable. Signing PRIVATE keys now live ONLY in GitHub Actions secrets (base64 PKCS#8 PEM: `WAVE2_OFFLINE_KEY_B64` / `WAVE4_MLDSA65_KEY_B64`), with a local backup in the maintainer's password manager; the repo holds ONLY public keys (`install/keys/*.pub*`) + their inlined verifier copies. Rotation = new keypair → `gh secret set` → swap the public keys + inline verifier blocks + fingerprints. **Enforced fail-closed by `scripts/ci/check_no_private_keys.py` (pre-commit + pre-push):** a full PEM PRIVATE KEY block, a tracked `*.key`/`*.key.enc`, or a `WAVE2_CEREMONY_PASSPHRASE=<value>` line blocks the commit/push. Doctrine: `docs/supply-chain/wave2-key-ceremony.md`. |
...
9. **Hallucinating-ASR wake precision/recall trap** (AP-27, BUG-037): the `stt_match` wake transcribes with a bias prompt; the primed model INVENTS the phrase on silence and GARBLES it on speech, so **"fires on silence" and "stops working" share ONE root** and any transcript-content ghost filter that tightens the wake also rejects real wakes. Signal: a "ghost fix" lands and the wake stops triggering for a hard custom word (`Mythos`→`Mütos`), or vice-versa. Defense: gate silence on raw ENERGY (word-agnostic RMS at the match site), NEVER on transcript content; keep the confirm permissive + skip it when loud (latency); the recall guard test must stay green. Truly-instant + zero-ghost custom wake needs a neural KWS model, not transcription (AP-25). <!-- i18n-allow: forensic quote of the German STT-garble token under test -->
