...
**(B) Maintainer identity** — real name in any spelling/casing (`Alex`/`Rubén`/`Maintainer`/`Maintainer`), alt name (`Sam`/`Sam Herz`), personal GitHub login (`octocat`), private emails (`alex.maintainer10@gmail.com`, `sam.herz@gmx.de`), `owner: <personal-name>` frontmatter.
