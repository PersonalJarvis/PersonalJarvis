# README hero one-liner — proposals

The current hero line ("A voice-driven meta-orchestrator that turns one spoken
request into a fleet of self-checking AI agents.") describes the architecture,
not the benefit — insider vocabulary ("meta-orchestrator", "fleet",
"self-checking") that means nothing to someone who just landed on the repo.
These three candidates lead with the promise instead. English is the README
language; a German translation is included for each so the same line can be
reused in German-language posts.

---

## Variant 1 — the plain promise (Recommended)
...
scale of it (full command of your PC) — no term requires prior knowledge, and
it mirrors exactly what the demo video shows. Variant 3 is punchier but leans
on the film's slogan, which is cryptic without the video context.

## Variant 2 — the butler frame
...
**Status:** Variant 1 is already applied to `README.md`. Swapping to another
variant is a one-line edit of the `<b>…</b>` hero line.
