...
3. **The current owners of `personal-jarvis/PersonalJarvis`** — at the
...
|---|---|---|---|
| `EXPECTED_REPO` | `personal-jarvis/PersonalJarvis` | The GitHub repo whose Actions identity is allowed to sign. | An attacker who signs a `install.sh` under a *different* repo's OIDC identity (even with a legitimately-issued Fulcio cert) will fail verification. |
...
| `install/keys/offline-ceremony.key.enc` | Ed25519 private key, encrypted with AES-256-CBC + PBKDF2 (600 000 iterations). **Not tracked in the public release** — only the `.pub` half is committed; the encrypted private blob is held off-repo. |
...
### 3.3 The passphrase — ROTATED OUT FOR THE PUBLIC RELEASE

> **Read this paragraph carefully.** It is the honesty bar for this entire
> branch.

During the original **Wave 2 foundation demo**, a real passphrase was
committed here in plain text to demonstrate the 2-of-2 verifier machinery.
For the public release that demo passphrase has been **rotated out and
removed** — the line now reads:

```
WAVE2_CEREMONY_PASSPHRASE=<DEMO-PASSPHRASE-ROTATED-OUT>
```

The original value is **burned** (never reused for any production key), and
the encrypted private key it once unlocked
(`install/keys/offline-ceremony.key.enc`) is **not tracked in this repository**
— only the public `.pub` half is committed. A reader of the public GitHub
repository can therefore extract **nothing**: there is no encrypted private
blob present and no live passphrase.
...
2. **Store the production passphrase as a GitHub Actions repository secret.**
   ...
   ```
   After setting, `gh secret list --repo personal-jarvis/PersonalJarvis`
...
| `EXPECTED_SLSA_SOURCE_URI` | `github.com/personal-jarvis/PersonalJarvis` | Source URI passed to `slsa-verifier verify-artifact`. |
| `EXPECTED_INTOTO_IDENTITY_REGEXP` | `^https://github\.com/personal-jarvis/PersonalJarvis/\.github/workflows/sign-installer\.yml@refs/tags/v[0-9]+\.[0-9]+\.[0-9]+(-[A-Za-z0-9._-]+)?$` | Functionary identity_regexp the in-toto layout MUST pin. Catch-all values (`.*`, `.+`, `^.*$`, `^.+$`, `""`) are rejected explicitly. |
...
```
https://github.com/personal-jarvis/PersonalJarvis/.github/workflows/sign-installer.yml@refs/tags/<TAG>
```
...
- The *person* who pushed the tag is recorded in Rekor but is not gated
  ...
  `personal-jarvis/PersonalJarvis` and the ability to push a `v*.*.*`
...
https://github.com/personal-jarvis/PersonalJarvis/settings/access. The
...
- **Secret scanning + push protection** enabled at the repo level via
  ...
  /repos/personal-jarvis/PersonalJarvis -F
