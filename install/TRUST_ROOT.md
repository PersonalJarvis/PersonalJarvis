...
Today (Wave 1), the maintainer set is the PersonalJarvis organization plus any
...
|---|---|---|---|
| 2026-05-26 | Initial Wave 1 pin: cosign `v2.4.1`, Linux/Darwin/Windows hashes, action SHAs (checkout `11bd7190…`, cosign-installer `dc72c7d5…`, action-gh-release `c95fe148…`). | Personal Jarvis maintainers + automated audit | `cosign_checksums.txt` for v2.4.1 fetched from `github.com/sigstore/cosign/releases/download/v2.4.1/cosign_checksums.txt`; action SHAs resolved via `api.github.com/repos/<org>/<repo>/git/refs/tags/<tag>`. |
...
- **Bot-identity migration** is explicitly OUT OF SCOPE for Wave 5.
  The signing actor remains the designated release maintainer (not a
